...
AsPlatform_Laser::AsPlatform_Laser(AsPlatform_State &platform_state)
	: Enable_Laser_Firing(false), Laser_Transformation_Step(0), Last_Laser_Shot_Tick(0), Platform_State(&platform_state), Platform_Circle_Color(0), Laser_Beam_Set(0), Inner_Color(0), Gun_Color(0)
{
}
//------------------------------------------------------------------------------------------------------------
void AsPlatform_Laser::Init(AsLaser_Beam_Set *laser_beam_set, AColor &platform_inner_color, AColor &platform_circle_color, AColor &highlight_color)
{
...
void AsPlatform_Laser::Draw_Laser_Leg(HDC hdc, double x_pos, bool is_left)
{
	...
	{
		x = x_pos * AsConfig::D_Global_Scale + (AsConfig::Platform_Normal_Width - 6) * d_scale - 1.0;
		x_scale = -scale;
	}
	...
	POINT left_leg_points[7] =
	{
		...
		{ (int)(x + 2 * x_scale), (int)(y + 2 * d_scale)}, { (int)(x + 2 * x_scale), (int)y + (int)(y + (2.0 + 2.0 * ratio) * d_scale)},
...
//------------------------------------------------------------------------------------------------------------
void AsPlatform_Laser::Draw_Laser_Head(HDC hdc, double x_pos)
{
	double x, y;
	double one_pixel_corr = 1.0 / AsConfig::D_Global_Scale;
	double ratio = (double)Laser_Transformation_Step / (double)Max_Laser_Transformation_Step;
	const int scale = AsConfig::Global_Scale;
	...
	Inner_Color->Select(hdc);
	Draw_Expanding_Figure(hdc, EFigure_Type::Ellipse, x + 13, y + 1, 2, 1, ratio, x + 9, y - 1, 10, 8 - one_pixel_corr);

	// BG head
	AsConfig::BG_Color.Select(hdc);
	Draw_Expanding_Figure(hdc, EFigure_Type::Rectangle, x + 13, y + 1, 2, 1, ratio, x + 11, y, 6, 1);
	Draw_Expanding_Figure(hdc, EFigure_Type::Ellipse, x + 13, y + 1, 2, 1, ratio, x + 10, y, 8, 5 - one_pixel_corr);

	// Main Head
	AsConfig::White_Color.Select(hdc);
	Draw_Expanding_Figure(hdc, EFigure_Type::Ellipse, x + 13, y + 1, 2, 1, ratio, x + 11, y, 6, 4 - one_pixel_corr);
}
//------------------------------------------------------------------------------------------------------------
