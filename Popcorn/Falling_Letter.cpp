...
AFalling_Letter::AFalling_Letter(EBrick_Type brick_type, ELetter_Type letter_type, int x, int y)
   : Brick_Type(brick_type), Letter_Type(letter_type), X(x), Y(y), Rotation_Step(2), Next_Rotation_Tick(AsConfig::Current_Timer_Tick + Ticks_Per_Step),
   Falling_Letter_State(EFalling_Letter_State::Normal)
{
   Letter_Cell.left = X;
   Letter_Cell.top = Y;
...
void AFalling_Letter::Test_Draw_All_Steps(HDC hdc)
{
   ...
   x_step = AsConfig::Cell_Width * AsConfig::Global_Scale;
   Rotation_Step = 0;
...
//-------------------------------------------------------------------------------------------------------------------------
void AFalling_Letter::Set_Brick_Letter_Color(bool is_switch_color, const AColor **front_color, const AColor **back_color)
{
   if (is_switch_color)
   {
      *front_color = &AsConfig::Orange_Color;
      *back_color = &AsConfig::Blue_Color;
   }
   else
   {
      *front_color = &AsConfig::Blue_Color;
      *back_color = &AsConfig::Orange_Color;
   }
}
//-------------------------------------------------------------------------------------------------------------------------
void AFalling_Letter::Draw_Brick_Letter(HDC hdc)
{// Отрисовка буквы крипича

   bool switch_color;
   double offset;
   double rotation_angle = 2.0 * M_PI / (double)Max_Rotation_Step * (double)Rotation_Step; // Преобразование шаг в угол поворота
   double y_ratio;
   int back_part_offset;
   const AColor* front_color, *back_color;
   XFORM xform, old_xform;

   if (!(Brick_Type == EBrick_Type::Blue || Brick_Type == EBrick_Type::Orange))
   ...
   {
      if (Brick_Type == EBrick_Type::Orange)
         switch_color = true;
      else
         switch_color = false;
   }
   ...
   else
   {
      ...
      offset = 3.0f * (1.0 - fabs(xform.eM22)) * AsConfig::D_Global_Scale;
      ...
      if (Rotation_Step > 4 && Rotation_Step <= 12)
      {
         ...
         {
         case ELetter_Type::O:
            Ellipse(hdc, 0 + 5 * AsConfig::Global_Scale, 1 * AsConfig::Global_Scale - Brick_Half_Height, 0 + 10 * AsConfig::Global_Scale,
               6 * AsConfig::Global_Scale - Brick_Half_Height - 1);
            break;
