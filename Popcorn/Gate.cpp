...
const double AGate::Gap_Height_Short_Step = Max_Gap_Short_Height / ((double)AsConfig::FPS / 2.0); // Для анимации за 0.5 сек
const double AGate::Max_Gap_Long_Height = 18.0;
const double AGate::Gap_Height_Long_Step = Max_Gap_Long_Height / ((double)AsConfig::FPS * 1.5); // Для анимации за 3 сек
//-------------------------------------------------------------------------------------------------------------------------
AGate::AGate(int x_pos, int y_pos, int level_x_pos, int level_y_pos)
   : Gate_State(EGate_State::Closed), Gate_Transformation(EGate_Transformation::Unknown), Edge_Count(5), X_Pos(x_pos), Y_Pos(y_pos), Origin_Y_Pos(y_pos), 
   Level_X_Pos(level_x_pos), Level_Y_Pos(level_y_pos), Gate_Close_Tick(0), Gap_Height(0.0)
...
void AGate::Draw_Cup(HDC hdc, bool top_cup)
{
   ...
   AsTools::Rect(hdc, x + 4, y + 3, 2, 2, AsConfig::Blue_Color);  //  "Заплатка" в углу
   AsTools::Rect(hdc, x + 4, y + 3, 1, 1, AsConfig::BG_Color);  //  Перфорация
   AsTools::Rect(hdc, x + 2, y, 2, 1, AsConfig::Blue_Color);  //  Хвост фигуры
