...
//-------------------------------------------------------------------------------------------------------------------------
class AsLevel: public AHit_Checker, public AGrafics_Object
{
public:
	~AsLevel();
   AsLevel();
...
private:
	...
	void Draw_Parachute_In_Level(HDC hdc, RECT& brick_type);
	void Draw_Parachute_Part(HDC hdc, RECT& brick_rect, int offset, int width);
	void Clear_Objects(HDC hdc, RECT& paint_area, AGrafics_Object** objects_array, int object_max_count);
	void Delete_Objects(AGrafics_Object** objects_array, int& object_count, int object_max_count);
	void Draw_Objects(HDC hdc, RECT &paint_area, AGrafics_Object **objects_array, int object_max_count);
	void Act_Objects(AGrafics_Object **objects_array, int &object_count, const int object_max_count);
