...
AColor AActive_Brick_Blue_Orange::Fading_Orange_Brick_Colors[Max_Fade_Step]; 
AColor AActive_Brick_Blue_Orange::Fading_Blue_Brick_Colors[Max_Fade_Step];
// AActive_Brick_Blue_Orange
//-------------------------------------------------------------------------------------------------------------------------
AActive_Brick_Blue_Orange::~AActive_Brick_Blue_Orange()
{
}
//-------------------------------------------------------------------------------------------------------------------------
AActive_Brick_Blue_Orange::AActive_Brick_Blue_Orange(EBrick_Type brick_type, int level_x, int level_y)
...
//-------------------------------------------------------------------------------------------------------------------------
void AActive_Brick_Blue_Orange::Act()
{
...
//-------------------------------------------------------------------------------------------------------------------------
void AActive_Brick_Blue_Orange::Draw(HDC hdc, RECT& paint_area)
{
   AColor *color = 0;
   ...
   case EBrick_Type::Orange:
      color = &Fading_Orange_Brick_Colors[Fade_Step];
      break;
...
//-------------------------------------------------------------------------------------------------------------------------
void AActive_Brick_Blue_Orange::Setup_Color()
{
   int i;

   for (i = 0; i < Max_Fade_Step; i++)
   {
      Get_Fading_Color(AsConfig::Orange_Color, i, Fading_Orange_Brick_Colors[i]);
      Get_Fading_Color(AsConfig::Blue_Color, i, Fading_Blue_Brick_Colors[i]);
   }
}
//-------------------------------------------------------------------------------------------------------------------------
void AActive_Brick_Blue_Orange::Draw_In_Level(HDC hdc, RECT &brick_rect, EBrick_Type brick_type)
...
//-------------------------------------------------------------------------------------------------------------------------
unsigned char AActive_Brick_Blue_Orange::Get_Fading_Channel(unsigned char color, unsigned char bg_color, int step)
{
   return color - step * (color - bg_color) / (Max_Fade_Step - 1);
}
//-------------------------------------------------------------------------------------------------------------------------
void AActive_Brick_Blue_Orange::Get_Fading_Color(const AColor &origin_color, int step, AColor &result_color)
{
   unsigned char r, g, b;
...
void AActive_Brick_Unbreakable::Draw(HDC hdc, RECT& paint_area)
{
   ...
   LineTo(hdc, Brick_Rect.left + 13 * scale - 1 + offset, Brick_Rect.top - scale);

   AsConfig::Unbreakable_Orange_Highlight.Select_Pen(hdc);
   MoveToEx(hdc, Brick_Rect.left + 6 * scale + offset, Brick_Rect.bottom + scale, 0);
   LineTo(hdc, Brick_Rect.left + 15 * scale - 1 + offset, Brick_Rect.top - scale);
...
void AActive_Brick_Multihit::Draw(HDC hdc, RECT& paint_area)
{
   ...
   step = Rotation_Step % Steps_Per_Tern;
...
{ // Вывод multihit кирпича на уровень
   ...
   AsConfig::Orange_Color.Select(hdc);
...
void AActive_Brick_Teleport::Act()
{
   ...
   else
   {
      ...
      case ETeleport_State::Starting:
         Animation_Step = 0;
         Teleport_State = ETeleport_State::Finihsing;
      ...
      case ETeleport_State::Finihsing:
         ...
         if (Ball != 0)
         {
            ...
            Ball = 0; // Отмечаем мячик как отсутствующий
            AsTools::Invalidate_Rect(Brick_Rect);
         }
         break;
      }
...
AAdvertisement::AAdvertisement(int level_x, int level_y, int width, int height)
   ...
   Ball_Width(Ball_Size * AsConfig::Global_Scale), Ball_Height(Ball_Size * AsConfig::Global_Scale), Ball_Y_Offset(0), Falling_Speed(0.0),
   Acceleration_Step(0.2), Brick_Regions(0)
{
   //int i, j;
...
void AAdvertisement::Act()
{
   ...
   // Заказываем перерисовку фрагментов над которыми пропали кирпичи
   for(i = 0; i < Height; i++)
      for(j = 0; j < Width; j++)
...
void AAdvertisement::Draw(HDC hdc, RECT& paint_area)
{
   ...
   deformaton = (int)((1.0 - Deformation_Ratio) * (double)scale * 2.0);

   ball_width = shadow_width + deformaton;
   ball_height = shadow_height - deformaton;
   ...
   // Рисуем края 
   AsConfig::Advertisement_Orange_Table.Select(hdc);
   ...
   // Рисуем шарик
   ball_width = Ball_Width + deformaton;
   ball_height = Ball_Height - deformaton;
...
void AAdvertisement::Show_Under_Brick(int level_x, int level_y)
{
   int x, y;
   RECT rect;
   int cellW = AsConfig::Cell_Width * AsConfig::Global_Scale;
   int cellH = AsConfig::Cell_Height * AsConfig::Global_Scale;
   ...
   rect.left = Ad_Rect.left + x * cellW;
   rect.top = Ad_Rect.top + y * cellH;
...
{ // Вывод неактивного кирпича на уровень
   ...
   for (i = 0; i < 2; i++)
   {
      ...
      // Рисуем блик
      AsConfig::White_Color.Select(hdc);
      Arc(hdc, x + scale, y + scale, x + (Circle_size - 1) * scale - 1, y + (Circle_size - 1) * scale - 1, x + 2 * scale, y + scale, x + scale, y + 3 * scale);
